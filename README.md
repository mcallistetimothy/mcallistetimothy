# Hi, I'm Maliyah Calliste Timothy 👋

🎓 **Artificial Intelligence & Business Analytics Student** at the University of South Florida

I'm passionate about using **data, technology, and AI** to help brands better understand their customers and make smarter business decisions.

My interests include:

* 📊 Consumer Insights
* 📈 Brand Analytics
* 📣 Marketing Analytics
* 🤖 AI for Business
* 📉 Business Intelligence

---

## 🚀 Currently Working On

* Consumer Insights projects using Python
* Excel dashboards for business performance analysis
* Building a portfolio focused on analytics for consumer brands
* Strengthening my SQL and Power BI skills

---

### 🛠️ Technical Skills

**Analytics & BI:** Python, SQL, Excel, Tableau, Power BI  
**Python:** Pandas, NumPy, Matplotlib, Scikit-learn  
**Methods:** EDA, Regression Analysis, Predictive Analytics, Data Visualization, Dashboard Development, KPI Analysis  
**Marketing Analytics:** Consumer Insights, Brand Analytics, A/B Testing, Audience & Behavioral Insights  
**Tools:** Git, GitHub, Jupyter Notebook, Google Colab, Jira

---

## 📂 Featured Projects

📌 **Beauty Industry Consumer Insights**

* Analyzed 15,000 beauty products using Python to identify factors influencing customer satisfaction, consumer engagement, and product performance.

📌 **Consumer Brand Performance Dashboard**

* Built an interactive Excel dashboard to evaluate sales performance and provide strategic recommendations.

📌 **Marketing Campaign Analysis**

* (In Progress)

---

## 🎯 Career Goals

I'm seeking opportunities in:

* Consumer Insights
* Brand Analytics
* Marketing Analytics
* Business Analytics
* Customer Insights

My long-term goal is to combine analytics, technology, and business strategy to help global brands make better, data-driven decisions.

---

## 🌎 Let's Connect

💼 LinkedIn:
https://www.linkedin.com/in/maliyah-calliste-timothy

📫 Feel free to connect if you'd like to discuss analytics, consumer behavior, AI, or brand strategy!
